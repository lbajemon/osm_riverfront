...
# we have one polygon for each studied city (n = 60)
...
raw_result = osmdata::opq(bbox = bbox_vec) %>%        osmdata::add_osm_feature(key = key, value = value) %>%
        osmdata::osmdata_sf()
...
for(i in 1:30){
  print(i)
  print(areas2$ville[i])
  save_osm_results(areas2$path[i])
}

# error for Brisbane (i = 18), use "sf_use_s2(FALSE)" before running the function for this entity 
```
